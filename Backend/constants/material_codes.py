"""Material codes mirrored from frontend Orders.tsx (PLC XML list)."""
...
DISALLOWED_MATERIAL_CODES = frozenset({"None", "000", ""})


def resolve_material_name(code: str) -> Optional[str]:
    code = str(code).strip()
    for item in MATERIAL_CODES:
        if item["code"] == code:
            return item["name"]
    return None
...
def is_valid_material_code(code: str) -> bool:
    code = str(code).strip()
    if not code or code in DISALLOWED_MATERIAL_CODES:
        return False
    return resolve_material_name(code) is not None
